...
class VulnrichmentRepositoryImpl(
    private val database: Database,
) : VulnrichmentRepository {
    ...
    override suspend fun getVulnrichmentData(cveId: String): VulnrichmentData? =
        dbQuery {
            VulnrichmentTable
                .selectAll()
                .where { VulnrichmentTable.cveId eq cveId }
    ...
    override suspend fun getVulnrichmentDataBatch(cveIds: List<String>): Map<String, VulnrichmentData> =
        dbQuery {
            ...
            VulnrichmentTable
                .selectAll()
                .where { VulnrichmentTable.cveId inList cveIds }
                .mapNotNull { toVulnrichmentData(it) }
    ...
    override suspend fun upsertVulnrichmentData(data: List<VulnrichmentData>) =
        dbQuery {
            ...
            data.chunked(500).forEach { chunk ->
                VulnrichmentTable.batchUpsert(
                    chunk,
                    VulnrichmentTable.cveId,
                    onUpdateExclude = listOf(VulnrichmentTable.createdAt),
                    ...
                    this[VulnrichmentTable.lastUpdated] = Instant.now()
                }
            }
    ...
    override suspend fun getStaleVulnrichmentIds(
