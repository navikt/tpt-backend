...
class VulnServiceImpl(
    ...
    private suspend fun fetchCveEnrichmentData(cveIds: List<String>): CveEnrichmentData {
        ...
        return CveEnrichmentData(kevCveIds, kevRansomwareCveIds, epssScores, nvdData, vulnrichmentData)
    }
    ...
    override suspend fun fetchGitHubVulnerabilitiesForUser(email: String, groups: List<String>): no.nav.tpt.domain.GitHubVulnResponse {
        ...
        val allVulnerabilitiesByRepo = gitHubRepository.getVulnerabilitiesByRepos(
            gitHubRepositoriesData.map { it.nameWithOwner }
        )

        val allCveIds = allVulnerabilitiesByRepo.values
            .flatten()
        ...
        gitHubRepositoriesData.forEach { repo ->
            val repoVulns = allVulnerabilitiesByRepo[repo.nameWithOwner] ?: emptyList()
            ...
            val vulnerabilities = repoVulns.mapNotNull { vuln ->
                ...
                val hasRansomwareCampaignUse = enrichmentData.kevRansomwareCveIds.contains(cveIdentifier)
                val cveData = enrichmentData.nvdData[cveIdentifier]
                val vulnrichmentEntry = enrichmentData.vulnrichmentData[cveIdentifier]
