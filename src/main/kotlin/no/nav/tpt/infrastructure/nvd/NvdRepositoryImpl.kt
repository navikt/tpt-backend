...
class NvdRepositoryImpl(
    ...
    override suspend fun getCveData(cveId: String): NvdCveData? =
        dbQuery {
            NvdCves
                .selectAll()
                .where { NvdCves.cveId eq cveId }
        ...
        dbQuery {
            ...
            cves.chunked(100).forEach { chunk ->
                ...
                NvdCves.batchUpsert(
                    chunk,
                    NvdCves.cveId,
                    onUpdateExclude = listOf(NvdCves.createdAt),
                ...
                updatedCount += existingCveIds.size
            }
    ...
    override suspend fun getCvesInKev(): List<NvdCveData> =
        dbQuery {
            NvdCves
                .selectAll()
                .where { NvdCves.cisaExploitAdd.isNotNull() }
                .mapNotNull { toNvdCveData(it) }
        }
